...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and': # and expressions
            ...
            # print("DEBUG:opertor",operator)
            # print("DEBUG:operands",operands)
            arguments = operands.map(calc_eval)# UPDATE THIS FOR Q2, what is type(operator)?
...
def calc_apply(op, args):
    # print("DEBUG:args",args.first)
    # print("DEBUG:args",args.rest)
    # op = calc_eval(op)
    # args = calc_eval(args)
    if op in bindings:
        op = bindings[op]
...
def floor_div(args):
    ...
    if args.rest.rest is nil:
        ...
        if type(args.rest.first) is Pair:
            args.rest.first = calc_eval(args.rest.first) 
        div = args.first  // args.rest.first
        return floor_div(Pair(div,args.rest.rest)) 
...
def eval_define(expressions):
    ...
    if expressions.rest.rest is nil:
        print("DEBUG 118 expression.rest",expressions.rest)
        print("DEBUG 118 expression.first",expressions.first)
        if expressions.rest.first in bindings:
            bindings[expressions.first] = bindings[expressions.rest.first] 
            return expressions.first
        else:
            print("DEBUG 118 expression.rest.first",expressions.rest.first)
