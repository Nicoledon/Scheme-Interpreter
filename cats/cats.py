...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    num = [item for item in paragraphs if select(item)]
    count = 0
    for item in num :
        if count  == k :
            return item
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    lenS  = List_size(source_words)
    lenT  = List_size(typed_words)
    count  = 0 
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list :
        return typed_word
    num = [diff_function(typed_word,item,limit) for item in word_list]
    if min(num) > limit :
        return typed_word
    size = List_size(num)
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def furry_fixes_helper(typed , source , count , limit):
        Slen = len(source)
        Tlen = len(typed)
        ...
        if count > limit :
            return count
        if typed[0] != source[0]:
           return  furry_fixes_helper(typed[1:],source[1:],count + 1 ,limit)
        return furry_fixes_helper(typed[1:],source[1:],count,limit)
    return furry_fixes_helper(typed,source,0,limit)    
    # END PROBLEM 6
...
def minimum_mewtations(typed, source, limit):
    ...
    3
    """
    if limit == -1:# Base cases should go here, you may add more base cases as needed.
        # BEGIN
        "*** YOUR CODE HERE ***"
        return 10000000
        # END
    ...
    if typed[0] == source[0]:
        ...
        # BEGIN
        "*** YOUR CODE HERE ***" 
        return min(min(minimum_mewtations(add[1:],source[1:],limit -1) , minimum_mewtations(remove,source,limit - 1) ), minimum_mewtations(substitute[1:],source[1:],limit -1)) + 1
        # END
